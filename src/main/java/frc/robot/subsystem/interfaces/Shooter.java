...
public class Shooter {
  ...
  private static final TalonFX mLeftShooterMotor =
      new TalonFX(Constants.CAN_IDS.LEFT_SHOOTER_MOTOR);
  private static final TalonFX mRightShooterMotor =
      new TalonFX(Constants.CAN_IDS.RIGHT_SHOOTER_MOTOR);

  public static final StatusSignal<Double> mLeftMotorVelo = mLeftShooterMotor.getVelocity();
  public static final StatusSignal<Double> mRightMotorVelo = mRightShooterMotor.getVelocity();
  ...
  public static void init() {
    ...
    leftShooterMotorConfig.CurrentLimits.SupplyCurrentLimit = 38;
    leftShooterMotorConfig.CurrentLimits.SupplyCurrentLimitEnable = true;
    leftShooterMotorConfig.CurrentLimits.SupplyCurrentThreshold = 120.0;
    leftShooterMotorConfig.CurrentLimits.SupplyTimeThreshold = 0.5;
    ...
    rightShooterMotorConfig.CurrentLimits.SupplyCurrentLimit = 38;
    rightShooterMotorConfig.CurrentLimits.SupplyCurrentLimitEnable = true;
    rightShooterMotorConfig.CurrentLimits.SupplyCurrentThreshold = 120.0;
    rightShooterMotorConfig.CurrentLimits.SupplyTimeThreshold = 0.5;
  ...
  public static double getLeftSpeed() {
    return Conversions.rotationalSpeedToLinearSpeed(
        mLeftShooterMotor.getVelocity().getValue(), (WHEEL_DIAMETER / 2.0));
  }

  public static double getRightSpeed() {
    return Conversions.rotationalSpeedToLinearSpeed(
        mRightShooterMotor.getVelocity().getValue(), (WHEEL_DIAMETER / 2.0));
  }
  ...
  public static void setShootSpeed(double speed) {
    ...
    mRightShooterMotor.setControl(
        new MotionMagicVelocityTorqueCurrentFOC(right)
            .withUpdateFreqHz(1000.0)
            .withEnableFOC(true));
    mLeftShooterMotor.setControl(
        new MotionMagicVelocityTorqueCurrentFOC(left).withUpdateFreqHz(1000.0).withEnableFOC(true));
  }
  ...
  public static boolean readyToShoot() {
    ...
    return ((mRightMotorVelo.getValue() > speed_thresh_right)
        && (mLeftMotorVelo.getValue() > speed_thresh_left));
  }
}
