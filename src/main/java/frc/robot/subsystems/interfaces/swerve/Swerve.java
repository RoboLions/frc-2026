...
public class Swerve {
  ...
  private class SwerveObjects {
    ...
    private static final SwerveRequest.FieldCentric teleopDrive =
        new SwerveRequest.FieldCentric()
            .withDeadband(SwerveConstants.MaxSpeed * 0.075)
            .withRotationalDeadband(SwerveConstants.MaxAngularRate * 0.075) // Add a 7.5% deadband
            .withDriveRequestType(DriveRequestType.Velocity);

    private static final SwerveRequest.FieldCentric closedLoopDrive =
        new SwerveRequest.FieldCentric()
            .withDeadband(SwerveConstants.MaxSpeed * 0.075)
            .withRotationalDeadband(SwerveConstants.MaxAngularRate * 0.075) // Add a 7.5% deadband
            .withDriveRequestType(DriveRequestType.Velocity);
  ...
  public static void disabledPeriodic() {
    SwerveObjects.Swerve
        .periodic(); // look at the function comment and see that this is actually just a
    // reorientation tool
  }
  ...
  public static void zeroCommand() {
    SwerveObjects.Swerve.setControl(
        SwerveObjects.teleopDrive.withVelocityX(0).withVelocityY(0).withRotationalRate(0));
  }
  ...
  public static void setFieldChassisSpeeds(ChassisSpeeds speeds) {
    SwerveObjects.Swerve.setControl(new SwerveRequest.ApplyFieldSpeeds().withSpeeds(speeds));
  }
  ...
  public static void addLimelightMeasurement(
      Pose2d inputPose, double timestampSeconds, Vector<N3> standardDeviations) {
