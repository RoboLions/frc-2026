...
public class Limelight {
  ...
  private static final int[] VALID_IDS = {
    1, 2, 3, 4, 5, 6, 8, 9, 10, 11, 13, 14, 17, 18, 19, 20, 21, 22, 24, 25, 26, 27, 29, 30
  };
  ...
  public static void periodic() {
    LimelightHelpers.setCameraPose_RobotSpace(
        FRONT_LEFT_CAM, FL_OFFSET.getX(), FL_OFFSET.getY(), FL_OFFSET.getZ(), 0, 15, 180);
    LimelightHelpers.setCameraPose_RobotSpace(
        FRONT_RIGHT_CAM, FR_OFFSET.getX(), FR_OFFSET.getY(), FR_OFFSET.getZ(), 0, 15, 180);
  ...
  private static void seedFromMegaTag1(String cameraName) {
    LimelightHelpers.SetRobotOrientation(
        cameraName, Swerve.getYawAsDegrees(), Swerve.getYawRateAsDeg(), 0, 0, 0, 0);
    ...
    if (isValid(mt1Pose)) {
      ...
      Swerve.addLimelightMeasurement(
          mt1Pose.pose, mt1Pose.timestampSeconds, VecBuilder.fill(xyStdDev, xyStdDev, rotStdDev));
  ...
  private static void updateWithMegaTag(String cameraName) {
    LimelightHelpers.SetRobotOrientation(
        cameraName, Swerve.getYawAsDegrees(), Swerve.getYawRateAsDeg(), 0, 0, 0, 0);
    LimelightHelpers.PoseEstimate mtPoseEstimate =
    ...
    if (isValid(mtPoseEstimate)) {
      ...
      if (Swerve.getYawRateAsDeg() > 180) {
        Logger.recordOutput(
            "Vision/ ERROR LOG: " + cameraName,
            "ROTATION TOO FAST, YAWRATE - " + Swerve.getYawRateAsDeg());
        return;
      }

      if (mtPoseEstimate.pose.getX() < 0
          || mtPoseEstimate.pose.getX() > 16.5
          || mtPoseEstimate.pose.getY() < 0
          || mtPoseEstimate.pose.getY() > 8.0) {
        return;
      }
